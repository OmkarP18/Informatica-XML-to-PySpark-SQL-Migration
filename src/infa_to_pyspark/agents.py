...
def validator(
    ...
    for label, pattern in placeholder_patterns.items():
        if re.search(pattern, pyspark_code or "", re.IGNORECASE):
            placeholder_hits.append(label)
            continue
        if sql_code and re.search(pattern, sql_code, re.IGNORECASE):
            placeholder_hits.append(label)
    if placeholder_hits:
        issues.append("Placeholder text detected: " + ", ".join(sorted(set(placeholder_hits))))
